@php
    ...
    $id = isset($dados['dados']['dados']) ? $dados['dados']['dados'] : false;
    // dd($dados);
@endphp
{{-- @if ($ac=='alt') --}}
<div class="row mb-4 ml-0 mr-0 ">
    @if ($status_contrato == 'Aprovado' || $status_contrato == 'aprovado')
            <div class="col-md-12 text-right">
                <button type="button" data-id="{{$id}}" title="{{__('Cantelar o contrato na sulamerica')}}" data-operacao="{{$numOperacao}}" onclick="cancelarSulamerica(this)" class="btn btn-outline-danger">
                    {{__('Cancelar')}}
                </button>
            </div>
    @else
